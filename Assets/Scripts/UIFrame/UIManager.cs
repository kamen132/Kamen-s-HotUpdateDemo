...
public class UIManager : Singleton<UIManager>
{
    ...
    public void OnUpdate()
    {
        for (int i = 0; i < m_WindowList.Count; i++)
        {
            ...
            if (m_WindowList[i] != null)
            {
                ...
                {
                    //判断是否是热更
                    HotFixFacade.Instance.appdomain.Invoke(window.HotFixClassName, "OnUpdate", window, null);
                }
                else
                {
    ...
    /// <returns></returns>
    public Window PopUpWnd(string wndName, bool bTop = true,bool resource=false, object param1=null,
        ...
        if (wnd == null)
        {
            ...
            if (m_RegisterDic.TryGetValue(wndName, out tp))
            {
                ...
                else
                {
                    ...
                    //改到热更  调用UI
                    wnd= HotFixFacade.Instance.appdomain.Instantiate<Window>(hotName);
                    wnd.isHotFix = true;
            ...
            {
                //判断是否是热更
                HotFixFacade.Instance.appdomain.Invoke(wnd.HotFixClassName, "Awake", wnd, param1,param2,param3);
            }
            else
            {
            ...
            {
                //判断是否是热更
                HotFixFacade.Instance.appdomain.Invoke(wnd.HotFixClassName, "OnShow", wnd, param1,param2,param3);
            }
            else
            {
    ...
    public void CloseWnd(Window window, bool destory = false)
    {
        if (window != null)
        {
            ...
            {
                //判断是否是热更
                HotFixFacade.Instance.appdomain.Invoke(window.HotFixClassName, "OnDisable", window, null);
                HotFixFacade.Instance.appdomain.Invoke(window.HotFixClassName, "OnClose", window, null);
            }
            else
            {
                window.OnDisable();
    ...
    public void HideWnd(Window wnd)
    {
        if (wnd != null)
        {
            ...
            {
                //判断是否是热更
                HotFixFacade.Instance.appdomain.Invoke(wnd.HotFixClassName, "OnDisable", wnd);
            }
            else
            {
                wnd.OnDisable();
    ...
    public void ShowWnd(Window wnd, bool bTop = true, params object[] paralist)
    {
        if (wnd != null)
        {
            ...
            {
                //判断是否是热更
                HotFixFacade.Instance.appdomain.Invoke(wnd.HotFixClassName, "OnShow", wnd, paralist);
            }
            else
            {
                wnd.OnShow(paralist);
